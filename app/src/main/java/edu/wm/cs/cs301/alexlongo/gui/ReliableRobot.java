...
import edu.wm.cs.cs301.alexlongo.generation.CardinalDirection;
import edu.wm.cs.cs301.alexlongo.generation.CardinalDirection;
import edu.wm.cs.cs301.alexlongo.generation.Maze;
import edu.wm.cs.cs301.alexlongo.gui.Constants.UserInput;
...
public class ReliableRobot implements Robot
{
	...
	// Project 7 rework
	Maze theMaze;
	StatePlaying theStatePlayingController;

	/**
	 * Setups the new Reliable Robot
	 */
	public void setup(int f, int b, int l, int r, Maze maze, StatePlaying sp)
	{
		theMaze = maze;
		theStatePlayingController = sp;
		if(f == 1)
		{
			frontSensor = new ReliableSensor(Direction.FORWARD);
		}
		else
		{
			frontSensor = new UnreliableSensor(Direction.FORWARD);
		}
		if(b == 1)
		{
			backSensor = new ReliableSensor(Direction.FORWARD);
		}
		else
		{
			backSensor = new UnreliableSensor(Direction.FORWARD);
		}
		if(l == 1)
		{
			leftSensor = new ReliableSensor(Direction.FORWARD);
		}
		else
		{
			leftSensor = new UnreliableSensor(Direction.FORWARD);
		}
		if(r == 1)
		{
			rightSensor = new ReliableSensor(Direction.FORWARD);
		}
		else
		{
			rightSensor = new UnreliableSensor(Direction.FORWARD);
		}
		soso = false;
	}
	 ...
	 * Setups the new Reliable Robot
	 */
	public void setup(int f, int b, int l, int r, Maze maze, StatePlaying sp, boolean soso2) {
		theMaze = maze;
		theStatePlayingController = sp;
		if (f == 1) {
			frontSensor = new ReliableSensor(Direction.FORWARD);
	...
	public void addDistanceSensor(DistanceSensor sensor, Robot.Direction mountedDirection)
	{
		...
		if(mountedDirection == Robot.Direction.LEFT)
		{
			leftSensor = sensor;
		...
		if(mountedDirection == Robot.Direction.RIGHT)
		{
			rightSensor = sensor;
	...
	public int[] getCurrentPosition() throws Exception 
	{
		...
		int[] currentPos = theStatePlayingController.getCurrentPosition();
		
		// check if the position is outside the maze
		if(((currentPos[0] < 0) || (currentPos[0] >= theMaze.getWidth())) || ((currentPos[1] < 0) || (currentPos[1] >= theMaze.getHeight())))
		{
	...
	public CardinalDirection getCurrentDirection()
	{
		...
		return theStatePlayingController.getCurrentDirection();
	}
	...
	public void rotate(Turn turn) 
	{
		...
		if(this.batteryLevel - 3 >= 0)
		{
			...
			{	
				...
				theStatePlayingController.handleUserInput("RIGHT");
			...
			{	
				// Use the controller to rotate the robot
				theStatePlayingController.handleUserInput("LEFT");
	...
	public void move(int distance) 
	{
		...
		for(int s = 0; s < distance; s++)
		{
			...
			if(this.batteryLevel - 6 >= 0)
			{
				...
				if(theMaze.hasWall(theStatePlayingController.getCurrentPosition()[0], theStatePlayingController.getCurrentPosition()[1], theStatePlayingController.getCurrentDirection()))
				{
				...
				else 
				{
					...
					theStatePlayingController.handleUserInput("UP");
	...
	{	
		...
		if(this.batteryLevel - 40 >= 0)
		{
			...
			if((theStatePlayingController.getCurrentDirection() == CardinalDirection.West) && (theStatePlayingController.getCurrentPosition()[0] == (theStatePlayingController.getMaze().getWidth() - 1)))
			{
			...
			else if((theStatePlayingController.getCurrentDirection() == CardinalDirection.East) && (theStatePlayingController.getCurrentPosition()[0] == 0))
			{
				// would jump over exterior wall, this is not supposed to happen, stop robot
				stopped = true;
			}
			// Check that the robot won't jump over a Northern exterior wall
			else if((theStatePlayingController.getCurrentDirection() == CardinalDirection.North) && (theStatePlayingController.getCurrentPosition()[1] == 0))
			{
				// would jump over exterior wall, this is not supposed to happen, stop robot
				stopped = true;
			}
			// Check that the robot won't jump over a Southern exterior wall
			else if((theStatePlayingController.getCurrentDirection() == CardinalDirection.South) && (theStatePlayingController.getCurrentPosition()[1] == (theStatePlayingController.getMaze().getHeight() - 1)))
			{
				// would jump over exterior wall, this is not supposed to happen, stop robot
				stopped = true;
			}
			...
			{
				// Use the controller to move the robot one step forward
				theStatePlayingController.handleUserInput("JUMP");
	...
	public boolean isAtExit() 
	{
		 ...
		 */
		if(theStatePlayingController.getCurrentPosition().equals(theMaze.getExitPosition()))
		{
			return true;
		}
	...
	public boolean isInsideRoom() 
	{
		 ...
		 * return false
		 */
		if(theMaze.isInRoom(theStatePlayingController.getCurrentPosition()[0], theStatePlayingController.getCurrentPosition()[1]))
		{
			return true;
		}
	...
	{		
		...
		if(direction == Direction.FORWARD)
		{
			...
			try 
			{
				...
				return frontSensor.distanceToObstacle(theStatePlayingController.getCurrentPosition(), theStatePlayingController.getCurrentDirection(), powersupply);
			} 
			catch (Exception e) 
			{
		...
		{
			backSensor.setMaze(theStatePlayingController.getMaze());
			...
			{
				// return the distance found by distance sensor
				return backSensor.distanceToObstacle(theStatePlayingController.getCurrentPosition(), theStatePlayingController.getCurrentDirection(), powersupply);
			} 
			catch (Exception e) 
			{
				// This error would come from distanceToObstacle method of frontSensor catching a powersupply error
		...
		{
			leftSensor.setMaze(theStatePlayingController.getMaze());
			...
			{
				// return the distance found by distance sensor
				return leftSensor.distanceToObstacle(theStatePlayingController.getCurrentPosition(), theStatePlayingController.getCurrentDirection(), powersupply);
			} 
			catch (Exception e) 
			{
				// This error would come from distanceToObstacle method of frontSensor catching a powersupply error
		...
		{
			rightSensor.setMaze(theStatePlayingController.getMaze());
